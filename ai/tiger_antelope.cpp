...
double getTransitionProbability( const CoordType & c1, size_t action, const
CoordType & c2 ) {
...
class GridWorld {
    public:
        ...
        double getTransitionProbability( size_t s, size_t a, size_t s1 ) const {
            //return ::getTransitionProbability( decodeState( s ), a,
decodeState( s1 ) ); if (11 == s || 12 == s) { return 0.0;
          }
          ...
          if (a == DOWN && movY >= 0) return 0.0;
           ...
           for (auto &obstacle : obstacles) {
                    if (p2.first == obstacle.first && p1.first ==
obstacle.second) { return 0.0;
                    }
              }
          double val = 1;
          return val;
...
<< "* "; else { bool printed = false; for (auto &obstacle : obstacles) { if (x
== obstacle.first && y == obstacle.second) { std::cout << "# "; printed = true;
                    }
              }
              if (printed == false) {
                      std::cout << ". ";
...
void printCurrentTimeString() {
    auto t =
std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()); auto tm
= *std::localtime(&t); std::cout << std::put_time(&tm, "%T");
}
*/
...
enum STATE { STATE_X = 0, STATE_Y, STATE_EMPTY };
...
int main() {
  ...
  for (auto it = validStates.begin() + 102; it != validStates.end(); ++it) {
    ...
    while (true) {
      ...
      for (int i = 0; i < A; ++i) {
        ...
        std::cout << "State: " << std::get<0>(ret)
                  << " reward: " << std::get<1>(ret) << " Action : " << i
                  << std::endl;
      ...
      std::cout << "Action chosen by me: " << std::get<1>(maxReward)
                << std::endl;
  ...
  /*
      ...
      auto rew = std::max({std::get<1>(model.sampleSR(0, 0)),
     std::get<1>(model.sampleSR(0, 1)), std::get<1>(model.sampleSR(0, 2))});
  ...
  // as the horizon here.
  // printCurrentTimeString(); std::cout << " - Solving MDP using infinite
  ...
  // printCurrentTimeString(); std::cout << " - Converged: " <<
  // std::get<0>(solution) << "\n";

  // AIToolbox::MDP::Policy policy(world.getS(), world.getA(),
  // std::get<1>(solution));
